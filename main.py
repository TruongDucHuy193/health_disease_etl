...
from app.etl.extract.extract import extract_heart_disease_data, convert_to_heart_disease_csv
from app.etl.transform.transform import transform_raw_heart_disease_data
from app.etl.load.load_to_csv import load_to_csv
...
def main():
    ...
    if (not args.extract_only and not args.transform_only and not args.merge_files and not args.all) or args.load_only:
        try:
            logging.info("Starting load process...")
              # Get transformed data - use new single file approach            logging.info("Starting transformation process...")            # Transform the merged raw data - auto-create if missing
            input_file = os.path.join(args.input_dir, 'raw_heart_disease.csv')
    ...
    if not args.extract_only and not args.load_only:
        try:
            ...
            # Transform the merged raw data - auto-create if missing
            input_file = os.path.join(args.input_dir, 'raw_heart_disease.csv')
            ...
            if args.transform_only:  # Only exit if this was the only operation
                return 1
      # Merge processed files option is deprecated (single file processing)
    if args.merge_files and not args.load_only:
        logging.warning("⚠️ --merge-files option is deprecated. Single file processing is used.")
        logging.info("💡 Use --load-only or --all to process data")
        return 0
